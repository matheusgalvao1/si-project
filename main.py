...
def QA(model_name):
    ...
    pretrained_model = os.path.join("models", model_name)
    ...
    for i in data_test['data']:
        for j in i['paragraphs']:
            for k in j['qas']:
                ...
                cont += 1
                print(f"{cont}, ID: {id_q}, Question:{question} Answer:{answers[id_q]}")
...
for i in models:
    ...
    try:
        Repo.clone_from("https://huggingface.co/deepset/" + i, os.path.join("models", i))
    except:
...
for i in models:
    pred = "preds_" + i + ".json"
    os.system('cmd /k "python evaluate-v2.0.py dev-v2.0.json ' + pred)

print("AVG TIME:")
for i in [0,1,2]:
    print(f"Model: {models[i]} | Time per question:{execution_times[i]}")
